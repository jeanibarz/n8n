...
describe('binary checks', () => {
	it('passes all checks for a valid workflow', () => {
		const feedback = runBinaryChecks(makeWorkflow(), ctx);
		const passRate = feedback.find((f) => f.metric === 'pass_rate');
		expect(passRate?.score).toBe(1);
	});

	it('fails has_nodes for empty workflow', () => {
		const feedback = runBinaryChecks(makeWorkflow({ nodes: [] }), ctx);
		const check = feedback.find((f) => f.metric === 'has_nodes');
		expect(check?.score).toBe(0);
	});

	it('fails has_trigger when no trigger node exists', () => {
		const workflow = makeWorkflow({
			nodes: [{ name: 'Set', type: 'n8n-nodes-base.set', parameters: {} }],
			connections: {},
		});
		const feedback = runBinaryChecks(workflow, ctx);
		const check = feedback.find((f) => f.metric === 'has_trigger');
		expect(check?.score).toBe(0);
	});

	it('fails all_nodes_connected for disconnected node', () => {
		const workflow = makeWorkflow({
			nodes: [
				{ name: 'Webhook', type: 'n8n-nodes-base.webhook' },
	...
	it('fails no_empty_set_nodes when Set has no assignments', () => {
		const workflow = makeWorkflow({
			nodes: [
				{ name: 'Webhook', type: 'n8n-nodes-base.webhook' },
	...
	it('fails no_disabled_nodes when a node is disabled', () => {
		const workflow = makeWorkflow({
			nodes: [
				{ name: 'Webhook', type: 'n8n-nodes-base.webhook' },
	...
	it('supports --only filter', () => {
		const feedback = runBinaryChecks(makeWorkflow(), ctx, { only: ['has_nodes'] });
		const metrics = feedback.filter((f) => f.kind === 'metric');
		expect(metrics).toHaveLength(1);
		expect(metrics[0].metric).toBe('has_nodes');
	});

	it('throws for unknown check name in --only filter', () => {
		expect(() => runBinaryChecks(makeWorkflow(), ctx, { only: ['bogus'] })).toThrow(
